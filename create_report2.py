...
def write_to_excel():
    ...
    def header():
        ...
        column_ex=6
        for i in pd.date_range(start='2019-06-01', end=date.today().strftime('%Y-%m-%d')):
    ...
    def airpay():
        ...
        def success():
            ...
            d = pd.DataFrame({'DATE': pd.date_range(start='2019-06-01', end=date.today().strftime('%Y-%m-%d')), 'num': 0.0})
            d = d.set_index('DATE')
            df1=d.join(df)
            while True:
                try:
        ...
        def fail(matching):
            ...
            d = pd.Series(pd.date_range(start='2019-06-01', end=date.today().strftime('%Y-%m-%d')))
